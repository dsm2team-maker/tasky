...
import {
  ...
  getTaskyInfoMessages,
  getUnreadTaskyInfoCount,
...
export const getTaskyInfoMessagesHandler = async (req: AuthRequest, res: Response) => {
  try {
    const data = await getTaskyInfoMessages(req.user!.userId);
    res.json({ success: true, data });
  } catch (error) {
    handleError(error, res);
  }
...
export const getUnreadTaskyInfoCountHandler = async (req: AuthRequest, res: Response) => {
  try {
    const count = await getUnreadTaskyInfoCount(req.user!.userId);
    res.json({ success: true, data: { count } });
  } catch (error) {
    handleError(error, res);
  }
